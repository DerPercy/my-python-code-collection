...
from services.stock_pool_service import buy_stocks,sell_stocks,get_pool_value, get_stock_with_lowest_indicator_value_at_datetime,get_stock_with_highest_indicator_value_at_datetime, get_portfolio_value
...
stocks.append( ("msci-health-care", "XDWH.DE"))
stocks.append( ("msci-tech",        "TNOW.PA"))
stocks.append( ("msci-telecom",     "WTEL.L"))
stocks.append( ("msci-cons-discr",  "XDWC.DE")) 
stocks.append( ("msci-industrials", "XDWI.DE")) 
...
for stock_price in pools[0].stock.prices:
    ...
    (max_pool,max_price) = get_stock_with_highest_indicator_value_at_datetime(pools,stock_price.datetime,"perf_1y")
    
    #if get_pool_value(max_pool.items,max_price) >= 100:
    #    sell_stocks(max_pool.items,max_price,100)
    #else:
...
content = template.render(
    ...
    f_get_pool_value=get_pool_value
)
with open("temp/report.html", mode="w", encoding="utf-8") as file:
    file.write(content)
